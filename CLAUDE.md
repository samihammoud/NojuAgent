...
- **On project load:** frontend calls `GET /api/files?project_id=...`. The saved `package.json` is written into WebContainer first, then `pnpm install` runs to sync `node_modules` with whatever the manifest declares. After that, the remaining source files are written. Vite never sees an unresolved import because all deps are already installed by the time any source file lands.
- **During agent turn:** `write_file` tool calls execute immediately in WebContainer (agent loop unblocked). `openFiles` is updated for editor UX but is no longer the source of truth for persistence.
- **After turn_complete:** frontend walks the live WebContainer FS via `snapshotFs()` ([useAgent.ts](ui/src/lib/useAgent.ts)) — listing all files (skipping `node_modules` and `pnpm-lock.yaml`) and reading each one. The resulting snapshot is both pushed to Supabase via `POST /api/files` and used to refresh `openFiles` for the editor. This single walk replaces the old `openFiles`-mirror + special-case `package.json` read + post-turn file refresh trio.
...
- **File persistence is frontend-owned, FS-backed.** The WebSocket has no knowledge of the DB. On `turn_complete`, the frontend walks the live WebContainer FS to build the snapshot (the FS is the source of truth — `openFiles` is just a derived view for the editor). On mount, it fetches `GET /api/files` and seeds WebContainer. This keeps the WebSocket handler stateless with respect to storage.
...
- **WebContainer boot is always eager and minimal.** `webcontainer.ts` calls `getWebContainer()` at module load — boot starts immediately with only the default `previewFiles` (react + react-dom). This keeps the initial "Installing dependencies" phase fast (~10–15s). The boot install runs against the *default* `package.json`; project-specific deps are installed afterward, on project load, by overwriting `package.json` with the saved version and re-running `pnpm install`.
- **WebContainer FS is the source of truth for persistence.** `openFiles` exists purely for the editor UI — it gets immediate updates on `write_file` so the user sees changes as they happen. But persistence goes through `snapshotFs(wc)`, which lists and reads every file directly from WebContainer. This automatically captures `pnpm add` / `pnpm install` side-effects on `package.json`, files written by shell commands, and any other in-FS state — without needing to mirror everything through `openFiles`.
- **Package restore order on load matters.** Manifest first, then deps, then source. The load sequence is: (1) write saved `package.json` into WebContainer, (2) `pnpm install` to sync `node_modules`, (3) write all other source files. Writing source files *before* the install causes Vite to error on unresolved imports because it tries to serve them immediately.
